...
Sections 18 (Land Valuation) & 19 (Builder/Developer Profitability) must compute, in a pro-forma TABLE, what a builder would PAY FOR THE LAND: start from ARV (from the comps' median $/sqft × planned GLA), subtract the localized total construction cost, subtract SITE-SPECIFIC adders that reduce land value — TREE/lot CLEARING (if wooded), extra GRADING/engineering (if slope ≥15%), and WELL + SEPTIC (if no public water/sewer) — subtract selling/closing/financing, and subtract a DEVELOPER PROFIT of 20% of ARV; the remainder is the RESIDUAL LAND VALUE. Cross-check it with the rule of thumb that builders pay ≈ 20% of ARV for a finished lot, THEN deduct those same site costs (trees, slope, well/septic), and present a defensible land-value RANGE. This is development-feasibility "what a builder would pay" — NOT a wholesale max-allowable-offer.
