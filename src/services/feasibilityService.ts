...
Present Section 16 (Development Cost Considerations) as an itemized TABLE mirroring this schedule with a LOCALIZED column (current local price + cited source) and a scaled TOTAL hard cost plus the resulting $/sqft. Always keep a builder fee and a 5–10% contingency.

## Developer Economics Standard (Sections 18 Land Valuation & 19 Builder/Developer Profitability)
...
Derive land value from comparable vacant-land sales, builder lot demand, new-construction economics, market absorption, and highest-and-best-use — NOT solely county tax values or automated estimates. Reconcile it with the Developer Economics residual land value above. Show the inputs and reasoning.
...
`;
  ...
  if (deepSeekKey && lastUser) {
    const [gDraft, dDraft] = await Promise.all([
      ...
      fetchDeepSeekDraft(`${systemPrompt}\n\n# PROVIDED DATA PACKET (verified evidence)\n${reportContext}\n\n# YOUR ROLE\nProvide a substantive but CONCISE expert analytical draft — your key findings, figures, and risks per topic (zoning, HOA/restrictions, buildability, flood, utilities, market, LOCAL itemized construction cost anchored to the Construction Cost Reference Model, and DEVELOPER ECONOMICS: ARV, residual land value = ARV − construction − site adders (trees/slope/well+septic) − 20%-of-ARV developer profit, cross-checked with the ~20%-of-ARV lot rule). A lead analyst synthesizes the final structured report from your input, so you need not format every numbered section.`, lastUser, deepSeekKey),
    ]);
