import type { SiteFeasibilityData, SlopeProfile, CompProperty, CompDateWindow, ResidentialCompType, FloodZoneInfo, WetlandsInfo, ConstructionCostEstimate, CostLineItem, MaterialTakeoff, MaterialTakeoffItem, LandClearingEstimate, TreeRemovalLine, ClearingMethod, UtilitiesEstimate, UtilityLine, PermitFeeLine } from '../types/feasibility';
...
import {
  ...
  residentialCompTypeLabel,
  zoningAllowedBuildingTypes as deriveZoningAllowedBuildingTypes,
  type ClassifiedCompType,
  type CompSearchFamily,
} from './comps/comp-types';
import {
...
export interface ParcelIdLookupResult {
  address: string;
  county: string;
  state: SupportedState;
  enteredParcelId: string;
  parcelId: string;
...
export function liveWebResearchConfigured(): boolean {
  return activeWebSearchProvider() !== 'none';
}
 ...
 *  contributes its second — same fairness the Perplexity batch path uses. */
function interleaveDedupe(groups: PplxResult[][]): PplxResult[] {
  ...
  for (let depth = 0; depth < maxDepth; depth++) {
    for (const group of groups) {
      const r = group[depth];
      if (r && !seen.has(r.url)) { seen.add(r.url); merged.push(r); }
    }
  }
  return merged;
}
...
async function perplexitySearchRequest(body: Record<string, unknown>, key: string): Promise<any | null> {
  ...
  const routes: { url: string; timeout: number }[] = [
    { url: '/.netlify/functions/perplexity', timeout: 10000 },
    { url: 'https://api.perplexity.ai/search', timeout: 10000 },
  ];
...
function parcelIdsFromAttributes(attributes: Record<string, any>): unknown[] {
  const normalized = normalizeCountyParcelAttrs(attributes);
  return [normalized.parno, normalized.pin, normalized.altparno]
    .filter((value) => value != null && String(value).trim() && String(value).trim().toUpperCase() !== 'N/A');
}

function parcelIdsFromFeature(feature: any): unknown[] {
...
function selectExpectedParcelFeature<T>(
  ...
  state: SupportedState,
  idsForFeature: (feature: T) => readonly unknown[] = parcelIdsFromFeature,
): T | null {
  return selectExactParcelFeature(
    features,
    expectedParcelIds,
    idsForFeature,
    state === 'SC',
...
function parcelIdMatchesExpected(
  parcelId: unknown,
  expectedParcelIds: readonly string[],
  state: SupportedState,
): boolean {
  if (!expectedParcelIds.length) return true;
  return expectedParcelIds.some((expected) =>
    parcelIdentitiesMatch(parcelId, expected, state === 'SC'),
  );
}
 ...
 */
async function queryCountyParcel(
  baseUrl: string,
  lng: number,
  lat: number,
  ...
  try {
    ...
    const pointFeatures: any[] = Array.isArray(wgsJson?.features) ? wgsJson.features : [];
    ...
    // point/envelope query must never contribute its owner or geometry.
    const exactFeature = expectedParcelIds.length
      ? selectExpectedParcelFeature(
    ...
    const pickedIds = parcelIdsFromFeature(wgsFeat).map((value) => String(value));
    const spMatch = pickedIds.length
      ? selectExpectedParcelFeature(spFeats, pickedIds, state)
      : null;
    const spFeat = spMatch || (!expectedParcelIds.length ? spFeats[pickedIdx] || spFeats[0] : null);
 ...
 */
async function queryMecklenburgParcel(
  lng: number,
  lat: number,
  expectedParcelIds: readonly string[] = [],
) {
  ...
  const result = await queryCountyParcel(boundary, lng, lat, undefined, expectedParcelIds, 'NC');
  ...
  try {
    ...
    const camaFeatures: any[] = (await res.json()).features || [];
    const camaFeature = expectedParcelIds.length
      ? selectExpectedParcelFeature(camaFeatures, expectedParcelIds, 'NC')
      : camaFeatures[0] || null;
    const camAttrs = camaFeature?.attributes;
    if (camAttrs) {
...
function normalizeParcelKey(v: unknown): string {
  return normalizeParcelIdentity(v);
}
...
interface RankedEsriParcelCandidate extends RankedParcelCandidate {
  esriFeature: any;
}

type InternalParcelLookupResult = ParcelIdLookupResult & { quality: number };

function exactLookupFeature(
  feature: any,
  state: SupportedState,
): ParcelIdLookupResult['feature'] {
  const properties = normalizeCountyParcelAttrs(feature?.attributes || {});
  if (state === 'SC') properties.recordsource = 'scdot';
  const rings = feature?.geometry?.rings;
  return {
    type: 'Feature',
    properties,
    geometry: Array.isArray(rings) && rings.length
      ? { type: 'Polygon', coordinates: rings }
      : null,
  };
}

async function lookupNcParcelById(
  pin: string,
  googleKey: string,
  countyHint?: string,
): Promise<InternalParcelLookupResult | null> {
  ...
  // Query the canonical PIN, county parcel ID, and separator-normalized forms.
  // Distinct matching parcels are reported as ambiguous, so formatting never
  // decides which owner is shown.
  const wheres = [
  ...
  const candidates: RankedEsriParcelCandidate[] = [];
  for (const where of wheres) {
    const url = `${NC_PARCEL_ENGINE}?where=${encodeURIComponent(where)}` +
      `&outFields=${encodeURIComponent(NC_PARCEL_FIELDS)}&returnGeometry=true&outSR=4326&resultRecordCount=25&f=json`;
    try {
      ...
      for (const f of feats) {
        ...
        const rawCounty = String(attrs.cntyname || '').trim();
        const properCounty = NC_COUNTY_NAMES.find((name) => name.toLowerCase() === rawCounty.toLowerCase()) || rawCounty;
        candidates.push({
          quality: r,
          state: 'NC',
          county: properCounty ? `${properCounty}, NC` : 'Unknown County, NC',
          parcelId: String(attrs.parno || attrs.altparno || pin).trim(),
          alternateParcelId: String(attrs.altparno || '').trim() || undefined,
          esriFeature: f,
        });
      }
    } catch { /* try the next WHERE variant */ }
  }
  const scopedCandidates = countyHint
    ? candidates.filter((candidate) =>
        countyBaseName(candidate.county).toLowerCase() === countyHint.toLowerCase(),
      )
    : candidates;
  const chosen = chooseUniqueTopParcelCandidate(pin, scopedCandidates);
  if (!chosen) return null;

  const feat = chosen.esriFeature;
  const a = feat.attributes || {};
  const county = chosen.county;
  const situs = String(a.siteadd || '').trim();
  const scity = String(a.scity || '').trim();
  const normalized = normalizeCountyParcelAttrs(a);
  const baseResult = {
    county,
    state: 'NC' as const,
    enteredParcelId: pin.trim(),
    parcelId: chosen.parcelId,
    alternateParcelId: chosen.alternateParcelId,
    ownerName: String(normalized.ownname || '').trim() || undefined,
    quality: chosen.quality,
    feature: exactLookupFeature(feat, 'NC'),
  };
  ...
  if (situs) return { ...baseResult, address: `${situs}${scity ? `, ${scity}` : ''}, NC`, lat, lng };
  ...
  if (typeof lat === 'number' && typeof lng === 'number' && googleKey) {
    try {
      ...
      if (gr.ok) {
        ...
        if (addr) return { ...baseResult, address: addr, lat, lng };
      }
    } catch { /* fall through */ }
  }
  return county
    ? { ...baseResult, address: `${county.replace(/,\s*NC$/i, '')} County, NC parcel ${chosen.parcelId}`, lat, lng }
    : null;
}
...
async function lookupScParcelById(
  pin: string,
  googleKey: string,
  countyHint?: string,
): Promise<InternalParcelLookupResult | null> {
  ...
  const inputKey = normalizeParcelKey(pin);
  if (!escaped.length || inputKey.length < 3) return null;
  // Some SC county portals append a display-only "000" account suffix while
  // SCDOT stores the same TMS without it (Union is one example). Query both
  // exact forms, then enforce the trailing-zero-only equivalence in code.
  const suffixlessKey = inputKey.length > 6 && inputKey.endsWith('000')
    ? inputKey.slice(0, -3)
    : null;
  const suffixlessDisplay = /[\s-]+000$/.test(inputUpper)
    ? inputUpper.replace(/[\s-]+000$/, '')
    : null;
  const lookupValues = [...new Set([
    ...escaped,
    ...(suffixlessKey ? [suffixlessKey] : []),
    ...(suffixlessDisplay ? [suffixlessDisplay.replace(/'/g, "''")] : []),
  ])];
  const inList = lookupValues.map((v) => `'${v}'`).join(', ');
  const wheres = [`UPPER(T_Map_Number) IN (${inList})`];
  // SCDOT rejects SQL REPLACE(), so use a bounded prefix/suffix fallback when
  // the user omits punctuation. Strict in-code identity matching still rejects
  // every non-equivalent result.
  const looseKey = suffixlessKey || (strippedEscaped ? inputKey : null);
  if (looseKey && looseKey.length >= 6) {
    wheres.push(`UPPER(T_Map_Number) LIKE '${looseKey.slice(0, 3)}%${looseKey.slice(-3)}'`);
  }

  const candidates: RankedEsriParcelCandidate[] = [];
  for (const where of wheres) {
    const url = `${SC_STATEWIDE_PARCEL_LAYER}/query?where=${encodeURIComponent(where)}` +
      `&outFields=${encodeURIComponent(SC_PARCEL_FIELDS)}&returnGeometry=true&outSR=4326&resultRecordCount=25&f=json`;
    try {
      const res = await fetchWithTimeout(url, 15000, { cache: 'no-store' });
      if (!res.ok) continue;
      const feats = (await res.json())?.features || [];
      for (const f of feats) {
        const storedParcelId = f?.attributes?.T_Map_Number;
        const r = parcelMatchRank(storedParcelId, '', inputUpper, inputKey) ||
          (parcelIdentitiesMatch(storedParcelId, pin, true) ? 1 : 0);
        if (!r) continue;
        const attrs = f?.attributes || {};
        const rawCounty = String(attrs.County || '').trim();
        const properCounty = SC_COUNTY_NAMES.find((name) => name.toLowerCase() === rawCounty.toLowerCase()) || rawCounty;
        candidates.push({
          quality: r,
          state: 'SC',
          county: properCounty ? `${properCounty}, SC` : 'Unknown County, SC',
          parcelId: String(attrs.T_Map_Number || pin).trim(),
          esriFeature: f,
        });
      }
    } catch { /* try the next WHERE variant */ }
  ...
  const county = chosen.county;
  const normalized = normalizeCountyParcelAttrs(feat.attributes || {});
  const baseResult = {
    county,
    state: 'SC' as const,
    enteredParcelId: pin.trim(),
    ...
    ownerName: String(normalized.ownname || '').trim() || undefined,
    quality: chosen.quality,
    feature: exactLookupFeature(feat, 'SC'),
  };
  let lat: number | undefined, lng: number | undefined;
  const ring: number[][] | undefined = feat.geometry?.rings?.[0];
  ...
  return county
    ? { ...baseResult, address: `${proper} County, SC parcel ${chosen.parcelId}`, lat, lng }
    : null;
}
...
export async function lookupParcelById(input: string, googleKey: string): Promise<ParcelIdLookupResult | null> {
  const parsed = parseParcelLookupInput(input);
  const pin = parsed.parcelId;
  const stateHint = parsed.stateHint as SupportedState | undefined;
  const countyNames = stateHint === 'SC' ? SC_COUNTY_NAMES : stateHint === 'NC' ? NC_COUNTY_NAMES : null;
  const countyHint = parsed.countyHint && countyNames
    ? countyNames.find((county) => county.toLowerCase() === parsed.countyHint?.toLowerCase())
    : parsed.countyHint;
  if (parsed.countyHint && stateHint && !countyHint) {
    throw new Error(`"${parsed.countyHint}" is not a ${stateHint} county. Use "parcel ID, County, ${stateHint}".`);
  }
  const [ncResult, scResult] = await Promise.allSettled([
    stateHint === 'SC' ? Promise.resolve(null) : lookupNcParcelById(pin, googleKey, countyHint),
    stateHint === 'NC' ? Promise.resolve(null) : lookupScParcelById(pin, googleKey, countyHint),
  ]);
  for (const result of [ncResult, scResult]) {
    if (result.status === 'rejected' && result.reason instanceof ParcelIdentityAmbiguityError) {
      throw result.reason;
    }
  }
  ...
  const best = chooseUniqueTopParcelCandidate(pin, hits);
  if (!best) return null;
  return {
    address: best.address,
    county: best.county,
    state: best.state,
    enteredParcelId: best.enteredParcelId,
    parcelId: best.parcelId,
    alternateParcelId: best.alternateParcelId,
...
export async function executeLandAnalysis(
  ...
  const selectedState = knownParcel?.state || addressState || countyState(countyName);
  ...
  countyName = `${countyBaseName(knownParcel?.county || countyName)}, ${selectedState}`;
  ...
  const expectedParcelIds = knownParcel
    ? [...new Set([
        ...
        knownParcel.parcelId,
        knownParcel.alternateParcelId,
      ].filter((value): value is string => !!value && !!normalizeParcelIdentity(value)))]
    : [];
  ...
  const exactCoords = knownCoords || (
    typeof knownParcel?.lat === 'number' && typeof knownParcel?.lng === 'number'
      ? { lat: knownParcel.lat, lng: knownParcel.lng }
      : undefined
  );
  ...
  if (hasScLocalParcelLayer && countyParcelLayer) {
    ...
    try {
      const localRes = await queryCountyParcel(
        countyParcelLayer,
        lng,
        lat,
  ...
  for (const parcelHost of parcelHosts.filter((v, i, arr) => arr.indexOf(v) === i)) {
    ...
    const directFeatures: any[] = Array.isArray(wgs84Data?.features) ? wgs84Data.features : [];
    const directExact = expectedParcelIds.length
      ? selectExpectedParcelFeature(directFeatures, expectedParcelIds, selectedState)
      : directFeatures[0] || null;

    // If no direct point intersection is found, or a parcel-ID search returned
    // only neighboring parcels, retry with a spatial envelope tolerance.
    if (!directFeatures.length || (expectedParcelIds.length && !directExact)) {
    ...
    if (wgs84Data && wgs84Data.features && wgs84Data.features.length > 0) {
      ...
      const exactFeature = expectedParcelIds.length
        ? selectExpectedParcelFeature(feats, expectedParcelIds, selectedState)
        : null;
      let pickedIdx = exactFeature ? feats.indexOf(exactFeature) : -1;
      if (!expectedParcelIds.length) {
        // A point near a property edge can intersect a roadway parcel. This
        // heuristic remains for address searches only; parcel-ID searches are
        // required to match the requested identity.
        pickedIdx = feats.findIndex((f) => !isRoadwayOwner(normalizeCountyParcelAttrs(f?.properties || {}).ownname));
      ...
      if (spFeats.length) {
        ...
        const pickedIds = parcelIdsFromFeature(parcelFeature).map((value) => String(value));
        const spMatch = pickedIds.length
          ? selectExpectedParcelFeature(spFeats, pickedIds, selectedState)
          : null;
        statePlaneFeature = spMatch || (!expectedParcelIds.length ? spFeats[pickedIdx] || spFeats[0] : null);
      } else {
  ...
  if (!parcelFeature && selectedState === 'NC' && (countyKeyLower === "mecklenburg" || countyParcelLayer)) {
    ...
    try {
      const localRes = countyKeyLower === "mecklenburg"
        ? await queryMecklenburgParcel(lng, lat, expectedParcelIds)
        : countyParcelLayer
          ? await queryCountyParcel(countyParcelLayer, lng, lat, undefined, expectedParcelIds, 'NC')
          : null;
      if (localRes) {
        parcelFeature = localRes.wgs84Feature;
        statePlaneFeature = localRes.statePlaneFeature;
  ...
  // The initial parcel-ID query already returned an exact feature. Keep it as
  // the final real-data fallback when point queries fail, but verify its ID
  // again before allowing any owner or geometry into the analysis.
  if (!parcelFeature && knownParcel?.feature) {
    const seedMatches = selectExpectedParcelFeature(
      [knownParcel.feature],
      expectedParcelIds,
      selectedState,
    );
    if (!seedMatches) {
      throw new Error(`The parcel service returned a feature that does not match parcel ID "${knownParcel.enteredParcelId}". No owner was applied.`);
    }
    parcelFeature = knownParcel.feature;
    statePlaneFeature = null;
    console.log(`${countyName} parcel resolved from the exact parcel-ID feature.`);
  }
  ...
  if (
    ...
    !parcelIdsFromAttributes(info).some((parcelId) =>
      parcelIdMatchesExpected(parcelId, expectedParcelIds, selectedState),
    )
  ) {
    throw new Error(`The resolved GIS record does not match parcel ID "${knownParcel?.enteredParcelId || expectedParcelIds[0]}". No owner was applied.`);
  }
  ...
  const rawCountyGisScRecord = selectedState === 'SC'
    ...
    : null;
  const countyGisScRecord = rawCountyGisScRecord?.status === 'verified' &&
    expectedParcelIds.length &&
    !parcelIdMatchesExpected(rawCountyGisScRecord.parcelId, expectedParcelIds, 'SC')
    ? null
    : rawCountyGisScRecord;
  if (rawCountyGisScRecord && !countyGisScRecord) {
    parcelConflicts.push('The county GIS owner record was rejected because its parcel ID did not match the requested parcel.');
  }
  ...
  const remoteOfficialScRecord = rawRemoteOfficialScRecord?.status === 'verified' &&
    expectedParcelIds.length &&
    !parcelIdMatchesExpected(rawRemoteOfficialScRecord.parcelId, expectedParcelIds, 'SC')
    ? null
    : rawRemoteOfficialScRecord;
  if (rawRemoteOfficialScRecord && !remoteOfficialScRecord) {
    parcelConflicts.push('The assessor or treasurer owner record was rejected because its parcel ID did not match the requested parcel.');
  }
  ...
  let zoningSetbacks: SiteFeasibilityData['zoningSetbacks'];
  let zoningMaxHeightFt: number | undefined;
  let zoningFloorAreaRatio: number | undefined;
  ...
  const buildGridics = () => {
    ...
    if (!zoningCode || !researched) return undefined;
    const valid = (value: unknown): value is number => Number.isFinite(Number(value));
    const required = [
      researched.setbacks?.frontFt,
      researched.setbacks?.rearFt,
      researched.setbacks?.sideFt,
      researched.maxHeightFt,
      researched.floorAreaRatio,
      researched.maxLotCoveragePct,
    ];
    if (!required.every(valid)) return undefined;
    const frontFt = Number(researched.setbacks?.frontFt);
    const rearFt = Number(researched.setbacks?.rearFt);
    const sideFt = Number(researched.setbacks?.sideFt);
    const maxHeightFt = Number(researched.maxHeightFt);
    const floorAreaRatio = Number(researched.floorAreaRatio);
    const coverage = Number(researched.maxLotCoveragePct) / 100;
    ...
    return {
      ...
      lotType: researched.lotType || 'district standards',
  ...
  try {
    ...
    const statusForZoningResult = (result: ZoningResult): SiteFeasibilityData['zoningVerificationStatus'] => {
      ...
      return 'listing-research';
    };
    const evidenceLabelForZoningResult = (result: ZoningResult) => {
    ...
    const applyResearchStandards = (research: ZoningResult) => {
      ...
      zoningSetbacks = research.standards?.setbacks ? {
        frontFt: Number.isFinite(Number(research.standards.setbacks.frontFt)) ? Number(research.standards.setbacks.frontFt) : undefined,
        rearFt: Number.isFinite(Number(research.standards.setbacks.rearFt)) ? Number(research.standards.setbacks.rearFt) : undefined,
        sideFt: Number.isFinite(Number(research.standards.setbacks.sideFt)) ? Number(research.standards.setbacks.sideFt) : undefined,
      } : undefined;
      zoningMaxHeightFt = Number.isFinite(Number(research.standards?.maxHeightFt)) ? Number(research.standards?.maxHeightFt) : undefined;
      zoningFloorAreaRatio = Number.isFinite(Number(research.standards?.floorAreaRatio)) ? Number(research.standards?.floorAreaRatio) : undefined;
      ...
      zoningSetbacksStatus = verifiedSetbacks === setbackValues.length
        ? 'official'
        : verifiedSetbacks > 0 ? 'mixed' : 'unavailable';
      const values = [
      ...
      zoningStandardsStatus = verifiedCount === values.length ? 'official' : verifiedCount > 0 ? 'mixed' : 'unavailable';
    };
  ...
  const zoningSupportsComps = !['review-required', 'planning-designation', 'unavailable'].includes(String(zoningVerificationStatus))
    && !/^(manual review|official map review|zoning code unresolved|land use:|no adopted district)/i.test(zoningCode.trim());
  ...
  const compAllowedTypes = zoningAllowedBuildingTypes(zoningPermittedUses, zoningCode, zoningDescription, zoningRestrictions);
  onPartial?.({ comps: compRun.comps, compRunSummary: compRun.summary, compAllowedTypes, compDateWindow: compRun.dateWindow });
  ...
  return {
    ...
    zoningPermittedUses,
    compAllowedTypes,
    compDateWindow: compRun.dateWindow,
    zoningTextReport,
    gridics,
    slopeProfile,
    floodZone,
...
export type CompBuildingType = ResidentialCompType;
...
export function zoningAllowedBuildingTypes(
  ...
  return deriveZoningAllowedBuildingTypes(permittedUses, zoningCode, zoningDesc, restrictions);
}
...
function parseZoningResult(text: string, evidenceUrls: string[] = [], countyName = ''): ZoningResult | null {
  ...
  try {
    ...
    if (ordinanceSource && evidenceUrlAllowed(ordinanceSource, evidenceUrls, countyName)) {
      const permittedUses = boundedStringList(obj?.permittedUses ?? standardsObj?.permittedUses, 24, 180);
...
function normalizeStreetKey(address: string): string {
  return compAddressMatchKey(address);
}
 ...
 *  Redfin / Zillow). Two records match when their canonical street and unit
 *  identities agree and their coordinates are nearby. Distinct condo, townhome,
 *  and mobile-home lot numbers remain separate. */
function dedupeComps(comps: any[]): any[] {
  ...
  for (const c of comps) {
    const cIdentity = compAddressIdentity(c.address);
    const idx = kept.findIndex((k) => {
      if (c.platform && k.platform && c.platform === k.platform && c.propertyId && c.propertyId === k.propertyId) return true;
      const kIdentity = compAddressIdentity(k.address);
      if (cIdentity.streetCore && kIdentity.streetCore && cIdentity.streetCore === kIdentity.streetCore) {
        if (cIdentity.unit !== kIdentity.unit) return false;
...
// search, server-filtered to Sold + new construction (the current and previous
// calendar year) within the rolling last 12 months; all three run in parallel and the
...
function realtorPropertyType(type: CompSearchFamily): string {
  const map: Record<CompSearchFamily, string> = {
    'single-family': 'House',
    townhouse: 'Townhome',
    condo: 'Condo',
...
function redfinHomeType(type: CompSearchFamily): string {
  const map: Record<CompSearchFamily, string> = {
    'single-family': 'House',
    townhouse: 'townhouse',
    condo: 'condo',
...
function prettyPropertyType(t: any): string | undefined {
  const classified = classifyCompBuildingType({ propertyType: t });
  if (classified.type === 'land') return 'Land';
  return classified.label;
}

// Comp property-type matching uses the source-backed taxonomy in
// services/comps/comp-types.ts.
...
function streetLineWithUnit(street: unknown, unit: unknown): unknown {
  if (!_isStr(street) || !_isStr(unit) || compAddressIdentity(String(street)).unit) return street;
  const cleanUnit = String(unit).trim().replace(/^(?:apt|apartment|unit|#)\s*/i, '');
  return cleanUnit ? `${String(street).trim()} Unit ${cleanUnit}` : street;
}
...
const COMP_CLASSIFICATION_PATH_RE = /(?:descriptiontext|marketingremark|publicremarks?|listingremarks?|propertytype(?:name)?|propertysubtype|hometype|buildingtype|structuretype|architecturalstyle|dwellingtype|usecode|usetype)/i;

/** Pull only property-form text from a source response. This intentionally
 * ignores generic descriptions (schools, agents, neighborhoods) so phrases such
 * as "multi-family students" cannot misclassify a listing. */
function collectCompClassificationText(node: any, path = '', out: string[] = [], depth = 0): string[] {
  if (node == null || depth > 10 || out.length >= 40) return out;
  if (Array.isArray(node)) {
    for (const value of node.slice(0, 12)) collectCompClassificationText(value, path, out, depth + 1);
    return out;
  }
  if (typeof node !== 'object') return out;
  for (const [key, value] of Object.entries(node)) {
    const nextPath = path ? `${path}.${key}` : key;
    const normalizedPath = _norm(nextPath);
    if (COMP_CLASSIFICATION_PATH_RE.test(normalizedPath) && (typeof value === 'string' || typeof value === 'number')) {
      const text = String(value).replace(/\s+/g, ' ').trim();
      if (text && text.length <= 4000 && !out.includes(text)) out.push(text);
    }
    if (value && typeof value === 'object') collectCompClassificationText(value, nextPath, out, depth + 1);
  }
  return out;
}

function compPublishedCount(raw: any, keyPattern: RegExp): number | undefined {
  const value = deepFind(raw, keyPattern, _isNum, 10);
  if (value === undefined) return undefined;
  const number = Math.round(_toNum(value));
  return Number.isInteger(number) && number > 0 ? number : undefined;
}

function normalizeRealtyListing(
  raw: any,
  platform: 'realtor' | 'redfin' | 'zillow',
  requestedFamily?: CompSearchFamily,
): any | null {
  ...
  let price: number | undefined, saleDate = "", yearBuilt: number | undefined,
      ...
      url: string | undefined, propertyId: any, listingId: any, rawStatus: any;
  ...
  if (platform === "realtor") {
    ...
    address = _join(streetLineWithUnit(a.line, a.unit), a.city, a.state_code ?? a.state, a.postal_code);
    ...
    if (la !== undefined && lo !== undefined) coords = { lat: la, lng: lo };
    address = _join(streetLineWithUnit(ai.formattedStreetLine, ai.unitNumber ?? ai.unit), ai.city, ai.state, ai.zip);
    ...
    if (la !== undefined && lo !== undefined) coords = { lat: la, lng: lo };
    address = _join(streetLineWithUnit(a.streetAddress, a.unit), a.city, a.state, a.zipcode);
    ...
    propertyId = pr.zpid;
    listingId = pr.listingId ?? pr.listing_id;
  }
  ...
  const unitCount = compPublishedCount(raw, /^(units|unitcount|numberofunits|totalunits|dwellingunits)$/);
  const structureCount = compPublishedCount(raw, /^(structurecount|numberofstructures|buildingcount|numberofbuildings|residentialbuildingcount)$/);
  const classificationText = collectCompClassificationText(raw).join(' | ');
  const classification = classifyCompBuildingType({
    propertyType,
    sourceText: classificationText,
    unitCount,
    structureCount,
    fallbackType: requestedFamily,
  });
  const compType = classification.type !== 'land' && classification.type !== 'unknown'
    ? classification.type
    : undefined;
  ...
  return {
    ...
    propertyType: classification.type === 'land' ? 'Land' : classification.label ?? prettyPropertyType(propertyType),
    compType,
    unitCount: classification.unitCount,
    structureCount: classification.structureCount,
...
function realtyHasNextPage(json: any): boolean {
  const v = deepFind(json, /(^nextpage$|hasnext|hasmore|morepages)/, (x) =>
    typeof x === 'boolean' || typeof x === 'number' || typeof x === 'string'
  );
  if (typeof v === 'boolean') return v;
  if (typeof v === 'number') return v > 0;
  return typeof v === 'string' && !/^(?:|false|0|null|none)$/i.test(v.trim());
}
...
async function fetchRealtyPlatform(
  ...
  lng: number,
  radiusMiles: number,
  requestedFamily: CompSearchFamily | undefined,
  allowedTypes: CompBuildingType[],
  dateWindow: CompDateWindow,
  key: string,
  onStageChange?: (stage: string) => void,
): Promise<any[]> {
  ...
  for (let page = 1; ; page++) {
    ...
    if (platform === "zillow") {
      ...
      q.set("yearBuiltRange", `min:${dateWindow.minYearBuilt},max:${dateWindow.maxYearBuilt}`);
      // Zillow ignores the year filter, so sort newest-built FIRST and page until
      // builds drop below the dynamic current/previous-year window
      ...
      if (platform === "realtor") {
        if (requestedFamily) q.set("propertyType", realtorPropertyType(requestedFamily));
        q.set("yearBuiltRange", `min:${dateWindow.minYearBuilt},max:${dateWindow.maxYearBuilt}`);
      } else {
        if (requestedFamily) q.set("homeType", redfinHomeType(requestedFamily));
        q.set("soldWithin", "Last_1_Year");
        q.set("minYearBuilt", String(dateWindow.minYearBuilt));
        q.set("maxYearBuilt", String(dateWindow.maxYearBuilt));
      }
    }
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const res = await fetchWithTimeout(`${REALTY_API_HOSTS[platform]}/search/bycoordinates?${q.toString()}`, 20000, {
          headers,
          cache: 'no-store',
        });
        if (res.ok) {
        ...
        if (attempt === 0 && transient) {
          onStageChange?.(`Retrying ${platform} ${requestedFamily || 'residential'} sold records...`);
    ...
    const pageSignature = listings.slice(0, 8).map((raw) => String(
      deepFind(raw, /(propertyid|listingid|zpid|fulladdress|streetaddress|formattedstreetline)/, (value) => _isStr(value) || _isNum(value)) ?? ''
    )).join('|') || `${page}:${listings.length}`;
    if (seenPageSignatures.has(pageSignature)) {
      console.warn(`RealtyAPI ${platform} repeated page ${page}; stopping pagination without duplicating results.`);
      break;
    }
    ...
    for (const raw of listings) {
      const c = normalizeRealtyListing(raw, platform, requestedFamily);
      if (!c) continue;
      ...
      // platform DOES expose the year, enforce the dynamic window.
      if (c.yearBuilt != null && (c.yearBuilt < dateWindow.minYearBuilt || c.yearBuilt > dateWindow.maxYearBuilt)) continue;
      c.newConstructionFlag = platform !== 'zillow';
      ...
      // Keep a broad Multi_Family candidate long enough to resolve its exact
      // unit count from the detail endpoint; the exact zoning gate runs later.
      if (!isSearchCompatibleCompType(c.compType ?? 'unknown', allowedTypes)) continue;
      out.push(c);
    }
    onStageChange?.(`Scanning ${platform} ${requestedFamily || 'all residential'} sold records... page ${page} (${out.length} found)`);

    if (!realtyHasNextPage(json)) break;
    if (platform === "zillow") {
      // Zillow is sorted newest-built first: once a whole page falls below the
      // dynamic floor, every later page is older too — stop paging.
      if (Number.isFinite(pageMaxYear) && pageMaxYear < dateWindow.minYearBuilt) break;
    } else if (Number.isFinite(pageOldest) && pageOldest < soldSince.getTime()) {
...
async function fetchRealtyApiSoldComps(
  ...
  const jobs: { platform: 'realtor' | 'redfin' | 'zillow'; family?: CompSearchFamily }[] = [
    ...
    // Zillow's current search wrapper does not publish a stable home-type enum;
    // run it once and apply the strict source-type gate client-side.
    { platform: 'zillow' as const },
  ];
  const perJob = await mapWithConcurrency(jobs, 4, async (job) =>
    fetchRealtyPlatform(
      job.platform,
      lat,
      lng,
      ...
      console.warn(`RealtyAPI ${job.platform}/${job.family || 'all'} search failed:`, error);
      return [] as any[];
    }),
  );
  ...
  for (const list of perJob) {
    for (const c of list) {
      ...
      const richerType = compClassificationSpecificity(c.compType ?? 'unknown') > compClassificationSpecificity(prev.compType ?? 'unknown')
        ? c
        : prev;
      const detailRefs = [...(winner.detailRefs || []), ...(filler.detailRefs || [])]
        .filter((ref: any, index: number, all: any[]) => all.findIndex((candidate) =>
          candidate.platform === ref.platform && candidate.propertyId === ref.propertyId && candidate.listingId === ref.listingId
        ) === index);
      byKey.set(k, {
        ...filler, ...winner,
        ...
        compType: richerType.compType,
        propertyType: richerType.propertyType,
        unitCount: richerType.unitCount,
        structureCount: richerType.structureCount,
        typeEvidence: richerType.typeEvidence,
        detailRefs,
      });
    }
  }
  const merged = Array.from(byKey.values());
  const counts = perJob.map((list, index) => `${jobs[index].platform}/${jobs[index].family || 'all'} ${list.length}`).join(", ");
  console.log(`RealtyAPI sold records: ${counts} -> ${merged.length} unique after cross-platform merge.`);
  return merged;
}

type RealtyDetailRef = {
  platform: 'realtor' | 'redfin' | 'zillow';
  propertyId: string;
  listingId?: string;
};

async function fetchRealtyDetail(ref: RealtyDetailRef, key: string): Promise<any | null> {
  let url = '';
  if (ref.platform === 'realtor') {
    url = `${REALTY_API_HOSTS.realtor}/details/byid?property_id=${encodeURIComponent(ref.propertyId)}`;
    if (ref.listingId) url += `&listing_id=${encodeURIComponent(ref.listingId)}`;
  } else if (ref.platform === 'redfin' && ref.listingId) {
    url = `${REALTY_API_HOSTS.redfin}/detailsbyid?property_id=${encodeURIComponent(ref.propertyId)}&listing_id=${encodeURIComponent(ref.listingId)}`;
  } else {
    return null;
  }
...
function familyFallbackForComp(type: ClassifiedCompType): CompSearchFamily | undefined {
  if (type === 'single-family' || type === 'mobile' || type === 'townhouse' || type === 'condo' || type === 'multi-family') return type;
  if (type === 'duplex' || type === 'triplex' || type === 'quadplex') return 'multi-family';
  return undefined;
}

function applyCompDetailClassification(comp: any, detail: any): any {
  const unitCount = compPublishedCount(detail, /^(units|unitcount|numberofunits|totalunits|dwellingunits)$/) ?? comp.unitCount;
  const structureCount = compPublishedCount(detail, /^(structurecount|numberofstructures|buildingcount|numberofbuildings|residentialbuildingcount)$/) ?? comp.structureCount;
  const sourceText = collectCompClassificationText(detail).join(' | ');
  const classification = classifyCompBuildingType({
    propertyType: comp.propertyType,
    sourceText,
    unitCount,
    structureCount,
    fallbackType: familyFallbackForComp(comp.compType ?? 'unknown'),
  });
  if (compClassificationSpecificity(classification.type) < compClassificationSpecificity(comp.compType ?? 'unknown')) return comp;
  if (classification.type === 'land' || classification.type === 'unknown') return comp;
  return {
    ...comp,
    compType: classification.type,
    propertyType: classification.label,
    unitCount: classification.unitCount,
    structureCount: classification.structureCount,
    typeEvidence: classification.evidence,
  };
...
/** Resolve broad Multi_Family records to duplex/triplex/quadplex/5+ units and,
 * when the ordinance allows it, inspect house records for multiple structures. */
async function enrichCompTypesFromDetails(
  comps: any[],
  allowedTypes: CompBuildingType[],
  key: string,
  onStageChange?: (stage: string) => void,
): Promise<any[]> {
  if (!key || comps.length === 0) return comps;
  const needsSmallMultifamily = allowedTypes.some((type) => ['duplex', 'triplex', 'quadplex'].includes(type));
  const needsMultipleStructures = allowedTypes.includes('multi-structure');
  if (!needsSmallMultifamily && !needsMultipleStructures) return comps;

  const targets = comps.filter((comp) =>
    (needsSmallMultifamily && comp.compType === 'multi-family') ||
    (needsMultipleStructures && (comp.compType === 'single-family' || comp.compType === 'multi-family'))
  );
  if (targets.length === 0) return comps;
  onStageChange?.(`Confirming dwelling-unit and structure counts for ${targets.length} zoning-matched candidate${targets.length === 1 ? '' : 's'}...`);

  const enriched = await mapWithConcurrency(targets, 4, async (comp) => {
    let current = comp;
    const refs = (Array.isArray(comp.detailRefs) ? comp.detailRefs : []) as RealtyDetailRef[];
    for (const ref of refs) {
      const detail = await fetchRealtyDetail(ref, key);
      if (!detail) continue;
      current = applyCompDetailClassification(current, detail);
      if (compClassificationSpecificity(current.compType ?? 'unknown') >= 3) break;
    }
    return current;
  });

  const byAddress = new Map(enriched.map((comp) => [normalizeStreetKey(comp.address), comp]));
  return comps.map((comp) => byAddress.get(normalizeStreetKey(comp.address)) ?? comp);
}
...
function buildCompRunSummary(opts: {
  ...
  radiusMiles: number;
  allowedTypes: ResidentialCompType[];
  dateWindow: CompDateWindow;
}): string {
  const { subjectAddress, comps, radiusExpanded, skippedZips, locations, candidateCount, scrapedCount, inRadiusCount, radiusMiles, allowedTypes, dateWindow } = opts;
  const lines: string[] = [];
  lines.push(`## 🏘️ New-Construction Sold Comp Run — ${subjectAddress}`);
  lines.push('');
  lines.push(`Criteria as of ${dateWindow.asOfDate}: new construction built ${dateWindow.minYearBuilt}-${dateWindow.maxYearBuilt}, matching the subject's source-backed zoning uses, sold on or after ${dateWindow.soldSinceDate}, no sqft limits, within ${radiusMiles} driving miles (every qualifying CLOSED sale in range, closest first). Sources: RealtyAPI closed-sale records — Realtor, Redfin & Zillow (coordinate radius scan; under-contract/pending excluded). Distances: Google Distance Matrix driving miles (straight-line in parentheses).`);
  lines.push('');
  lines.push(`Zoning-permitted residential forms searched: ${allowedTypes.map(residentialCompTypeLabel).join(', ')}.`);
  const typeCoverage = compCoverageTypesForAllowedTypes(allowedTypes).map((type) => {
    const count = comps.filter((comp) => comp.compType === type).length;
    return `${residentialCompTypeLabel(type)}: ${count}`;
  });
  lines.push(`Verified closing coverage by zoning-permitted form: ${typeCoverage.join(' · ')}. A zero means no qualifying source record was returned; the category was still searched.`);
  lines.push('');
  ...
  if (comps.length === 0) {
    const why = candidateCount === 0
      ? `None of the collected sold listings were built ${dateWindow.minYearBuilt}-${dateWindow.maxYearBuilt} and sold on or after ${dateWindow.soldSinceDate} — there may simply be no matching new-construction resale in this area yet.`
      : (inRadiusCount ?? 0) === 0
        ? `${candidateCount} new-construction sale${candidateCount === 1 ? '' : 's'} matched the spec, but none closed within ${radiusMiles} driving miles of the subject.`
  ...
  comps.forEach((c, i) => {
    ...
    if (c.typeEvidence) lines.push(`- Type evidence: ${c.typeEvidence}`);
    if (c.zoningMatchReason) lines.push(`- Zoning match: ${c.zoningMatchReason}`);
  ...
  try {
    ...
    if (run.comps.length > 0) {
      const rows = run.comps.map((c) => ({
        ...
        sources: 'RealtyAPI (Realtor/Redfin/Zillow)',
      }));
...
// the MLS figure and flagged. Every lookup is fresh so repeated address searches
// cannot reuse an older sold-price response.
// ---------------------------------------------------------------------------
...
/** A zpid's SOLD price-history events ({date,price}) from Zillow MLS data. */
async function fetchZillowSoldEvents(zpid: string, key: string): Promise<{ date: string; price: number }[] | null> {
  try {
    const res = await fetchWithTimeout(`${REALTY_API_HOSTS.zillow}/pricehistory?byzpid=${encodeURIComponent(zpid)}`, 15000, {
      headers: { "x-realtyapi-key": key },
      cache: 'no-store',
    });
    if (!res.ok) return null;
 ...
 * (typically a RealtyAPI record whose search payload omitted photos). For each photoless comp with
...
async function backfillCompPhotos(comps: CompProperty[], key: string): Promise<void> {
  ...
  const lookOn = async (platform: 'realtor' | 'zillow' | 'redfin', c: CompProperty): Promise<string[] | undefined> => {
    try {
      ...
      const res = await fetchWithTimeout(`${REALTY_API_HOSTS[platform]}/search/bycoordinates?${q.toString()}`, 12000, {
        headers: { 'x-realtyapi-key': key },
        cache: 'no-store',
      });
      if (!res.ok) return undefined;
  ...
  await mapWithConcurrency(targets, 4, async (c) => {
 ...
 * exterior (Gemini Vision over its photo set). Every comp is inspected; when
 * Vision cannot identify an exterior, the source cover remains available. */
async function selectExteriorComps(comps: CompProperty[], geminiKey: string): Promise<void> {
  if (!geminiKey) return;
  const targets = comps.filter((c) => Array.isArray(c.photoUrls) && c.photoUrls.length);
  if (!targets.length) return;
  let i = 0;
  const worker = async () => {
    while (i < targets.length) {
      ...
      if (idx >= 0 && idx < valid.length) c.imageUrl = valid[idx].u;
    }
  };
...
export async function fetchGoogleDistanceMatrixComps(
  ...
  const dateWindow = getRollingCompDateWindow();

  if (!zoningVerified || !zoningCode || /^(n\/?a|unknown|manual review|official map review|zoning code unresolved|land use:|no adopted district)/i.test(zoningCode.trim())) {
    return {
      comps: [],
  ...
  // NEW CONSTRUCTION (current and previous calendar year) matching zoning,
  ...
  const allowedTypes = zoningAllowedBuildingTypes(permittedUses, zoningCode, zoningDesc, zoningRestrictions);
  if (allowedTypes.length === 0) {
    return { comps: [], summary: "No residential comps run: this parcel's zoning is office/commercial, so residential sold records do not apply.", dateWindow };
  }
  ...
  const isNewConstruction = (yb: any) => {
    const y = Number(yb);
    return Number.isFinite(y) && y >= dateWindow.minYearBuilt && y <= dateWindow.maxYearBuilt;
  };
  const soldWithinYear = (sd?: string) => {
    if (!sd) return false;
    const d = new Date(sd);
    return !isNaN(d.getTime()) && d >= soldSince && d <= today;
  };
  ...
  const realtyComps = await fetchRealtyApiSoldComps(lat, lng, allowedTypes, dateWindow, onStageChange, EXPANDED_RADIUS_MILES).catch((e) => {
  ...
  // STEP 5 — filter to spec (no distance yet): sold, dynamically dated (zoning
  // use-category already applied per source), sold ≤12 months, price > 0.
  // NO sqft limits and no client-side result truncation.
  const candidates = compAddresses.filter((c: any) =>
    String(c.status || 'sold').toLowerCase().includes('sold') &&
    // New construction: year built in the dynamic window, or Realtor's official
  ...
  // Order by PROXIMITY (closest first) so the full selected-radius set is built from the
  ...
  if (finalCands.length < zoningMatchedCands.length) {
  ...
  const compsAll = finalCands.map((c, idx) => {
    ...
    return {
      ...
      structureCount: c.structureCount,
      typeEvidence: c.typeEvidence,
      zoningMatchReason: `${residentialCompTypeLabel(c.compType)} is included by the parcel's source-backed zoning use profile.`,
      coords: c.coords,
      sqft: c.sqft,
      url: c.url,
      zip: c.zip,
  ...
  // STEP 6c — include EVERY qualifying comp from the closest out to the full
  // selected driving radius, ordered nearest-first.
  ...
  // STEP 7 — source attribution. Provider records confirmed on their detail
  // endpoint get a verified badge; search-only records carry a confirm note.
  ...
  const result: CompProperty[] = verified
    ...
    .map((c: any) => {
      const { propertyId, listingId, detailRefs, propertyHistory, status, sourceName, newConstructionFlag, detailConfirmed, priceConfirmed, platform, ...rest } = c;
  ...
  // Backfill REAL listing photos for any comp whose RealtyAPI search record
  // omitted them. Best-effort.
  ...
  return { comps: result, summary, dateWindow };
}
...
interface TreeRates {
  ...
  smallLow: number; smallHigh: number;
  mediumLow: number; mediumHigh: number;
  largeLow: number; largeHigh: number;
  stumpGrindLow: number; stumpGrindHigh: number;
  mulchPerAcre: number; mulchDayRate: number;
  mulchPerAcreLow: number; mulchPerAcreHigh: number;
  mulchDayRateLow: number; mulchDayRateHigh: number;
  clearingPerAcre: number; clearingDayRate: number;
  clearingPerAcreLow: number; clearingPerAcreHigh: number;
  clearingDayRateLow: number; clearingDayRateHigh: number;
  haulOff: number;
  haulOffLow: number; haulOffHigh: number;
  sources: string[];
  scope: 'local' | 'regional';
}

function emptyTreeRates(): TreeRates {
  return {
    small: 0, medium: 0, large: 0, stumpGrind: 0,
    smallLow: 0, smallHigh: 0, mediumLow: 0, mediumHigh: 0,
    largeLow: 0, largeHigh: 0, stumpGrindLow: 0, stumpGrindHigh: 0,
    mulchPerAcre: 0, mulchDayRate: 0, clearingPerAcre: 0, clearingDayRate: 0, haulOff: 0,
    mulchPerAcreLow: 0, mulchPerAcreHigh: 0, mulchDayRateLow: 0, mulchDayRateHigh: 0,
    clearingPerAcreLow: 0, clearingPerAcreHigh: 0, clearingDayRateLow: 0, clearingDayRateHigh: 0,
    haulOffLow: 0, haulOffHigh: 0, sources: [], scope: 'regional',
  };
}
...
async function fetchTreeRemovalRates(county: string, zip: string, geminiKey: string): Promise<TreeRates | null> {
  ...
  const prompt = `Find CURRENT land-clearing budget ranges near ${locality} (${new Date().getFullYear()}).
Return ONLY a JSON object in a \`\`\`json code block:
\`\`\`json
{ "smallLow": 0, "smallHigh": 0, "mediumLow": 0, "mediumHigh": 0, "largeLow": 0, "largeHigh": 0, "stumpGrindLow": 0, "stumpGrindHigh": 0, "mulchPerAcreLow": 0, "mulchPerAcreHigh": 0, "mulchDayRateLow": 0, "mulchDayRateHigh": 0, "clearingPerAcreLow": 0, "clearingPerAcreHigh": 0, "clearingDayRateLow": 0, "clearingDayRateHigh": 0, "haulOffLow": 0, "haulOffHigh": 0, "sources": ["https://..."] }
\`\`\`
- small/medium/large low/high = remove ONE tree (<30ft / 30-60ft / 60-80+ft)
- stumpGrind low/high = grind one stump
- mulchPerAcre and mulchDayRate low/high = forestry mulching rates
- clearingPerAcre and clearingDayRate low/high = traditional excavator clearing with root removal
- haulOff low/high = debris haul-off / dump allowance for a small lot
Prefer current county/ZIP sources. If those pages do not publish prices, use current NC/SC regional contractor sources and make the range wide enough to contain the cited figures. Every nonzero range needs at least one URL from the supplied evidence. Do not use national cost aggregators and never invent a rate.`;
  ...
  const text = await groundedGeminiText(
    geminiKey,
    prompt,
    'You are a land-clearing pricing analyst. Use only the supplied live sources. Return current source-backed local or Carolinas regional ranges, not uncited fallback constants. Return only JSON.',
    60000,
    ...
    [
      ...
      `current tree removal price ranges ${stateFull}`,
      `current forestry mulching and site clearing rates ${stateFull}`,
    ],
    diag,
  );
  if (!text) return null;
  ...
  try {
    const o = JSON.parse((m[1] || m[0]).replace(/,\s*([}\\]])/g, '$1'));
    const range = (name: string): { low: number; high: number; midpoint: number } => {
      return normalizeSourcedRange(o[name], o[`${name}Low`], o[`${name}High`])
        || { low: 0, high: 0, midpoint: 0 };
    };
    const small = range('small');
    const medium = range('medium');
    const large = range('large');
    const stumpGrind = range('stumpGrind');
    const mulchPerAcre = range('mulchPerAcre');
    const mulchDayRate = range('mulchDayRate');
    const clearingPerAcre = range('clearingPerAcre');
    const clearingDayRate = range('clearingDayRate');
    const haulOff = range('haulOff');
    if (![small, medium, large, mulchPerAcre, clearingPerAcre].some((value) => value.midpoint > 0)) return null;
    const modelSources: string[] = Array.isArray(o.sources) ? o.sources.map((s: any) => String(s)) : [];
    const discoveredHosts = new Set((diag.perplexityUrls || []).map((url) => {
      try { return new URL(url).hostname.toLowerCase(); } catch { return ''; }
    }).filter(Boolean));
    const modelGrounded = modelSources.filter((url) => {
      try {
        const host = new URL(url).hostname.toLowerCase();
        return !NATIONAL_AGGREGATORS.some((blocked) => host.includes(blocked))
          && (!diag.perplexityAttempted || discoveredHosts.size === 0 || discoveredHosts.has(host));
      } catch { return false; }
    });
    if (modelGrounded.length === 0) return null;
    const sources: string[] = [...new Set<string>([
      ...modelGrounded,
      ...filterLocalSources(diag.perplexityUrls || [], [baseCounty, `${baseCounty.toLowerCase()}county`, stateFullLower, stateLower]),
    ])].slice(0, 8);
    if (!sources.length) return null;
    const compactLocality = `${baseCounty}${zip}`.toLowerCase().replace(/[^a-z0-9]/g, '');
    const scope = sources.some((url) => url.toLowerCase().replace(/[^a-z0-9]/g, '').includes(baseCounty.toLowerCase().replace(/[^a-z0-9]/g, ''))
      || (!!zip && url.includes(zip))
      || (!!compactLocality && url.toLowerCase().replace(/[^a-z0-9]/g, '').includes(compactLocality)))
      ? 'local'
      : 'regional';
    return {
      small: small.midpoint, smallLow: small.low, smallHigh: small.high,
      medium: medium.midpoint, mediumLow: medium.low, mediumHigh: medium.high,
      large: large.midpoint, largeLow: large.low, largeHigh: large.high,
      stumpGrind: stumpGrind.midpoint, stumpGrindLow: stumpGrind.low, stumpGrindHigh: stumpGrind.high,
      mulchPerAcre: mulchPerAcre.midpoint, mulchPerAcreLow: mulchPerAcre.low, mulchPerAcreHigh: mulchPerAcre.high,
      mulchDayRate: mulchDayRate.midpoint, mulchDayRateLow: mulchDayRate.low, mulchDayRateHigh: mulchDayRate.high,
      clearingPerAcre: clearingPerAcre.midpoint, clearingPerAcreLow: clearingPerAcre.low, clearingPerAcreHigh: clearingPerAcre.high,
      clearingDayRate: clearingDayRate.midpoint, clearingDayRateLow: clearingDayRate.low, clearingDayRateHigh: clearingDayRate.high,
      haulOff: haulOff.midpoint, haulOffLow: haulOff.low, haulOffHigh: haulOff.high,
      sources,
      scope,
    };
  } catch { return null; }
...
export async function fetchLandClearingEstimate(reportData: SiteFeasibilityData): Promise<LandClearingEstimate | null> {
  ...
  const rateRanges = {
    small: { low: r.smallLow, high: r.smallHigh, midpoint: r.small },
    medium: { low: r.mediumLow, high: r.mediumHigh, midpoint: r.medium },
    large: { low: r.largeLow, high: r.largeHigh, midpoint: r.large },
  };
  const trees: TreeRemovalLine[] = sizes
    .map((size) => ({
      size,
      count: vision[size],
      unitCost: rateRanges[size].midpoint,
      unitCostLow: rateRanges[size].low,
      unitCostHigh: rateRanges[size].high,
      cost: Math.round(vision[size] * rateRanges[size].midpoint),
      costLow: Math.round(vision[size] * rateRanges[size].low),
      costHigh: Math.round(vision[size] * rateRanges[size].high),
    }))
    .filter((tree) => tree.count > 0 && tree.unitCost > 0);
  const treeCount = vision.small + vision.medium + vision.large;
  const treeRemovalCost = trees.reduce((s, t) => s + t.cost, 0);
  const treeRemovalCostLow = trees.reduce((sum, tree) => sum + tree.costLow, 0);
  const treeRemovalCostHigh = trees.reduce((sum, tree) => sum + tree.costHigh, 0);
  const stumpGrindCost = Math.round(treeCount * r.stumpGrind);
  const stumpGrindCostLow = Math.round(treeCount * r.stumpGrindLow);
  const stumpGrindCostHigh = Math.round(treeCount * r.stumpGrindHigh);
  ...
  return {
    ...
    stumpGrindUnitLow: r.stumpGrindLow,
    stumpGrindUnitHigh: r.stumpGrindHigh,
    stumpGrindCost,
    stumpGrindCostLow,
    stumpGrindCostHigh,
    total: treeRemovalCost + stumpGrindCost,
    totalLow: treeRemovalCostLow + stumpGrindCostLow,
    totalHigh: treeRemovalCostHigh + stumpGrindCostHigh,
    clearingMethods,
    clearingFactors,
    satelliteUrl,
    ...
    pricingStatus: rates ? 'estimated' : 'unavailable',
    pricingScope: rates?.scope,
...
export async function fetchUtilitiesEstimate(reportData: SiteFeasibilityData): Promise<UtilitiesEstimate | null> {
  ...
  const prompt = `For the property at ${locality}, determine (A) whether the address is served by PUBLIC water and PUBLIC sewer, needs a private WELL / SEPTIC, or is a MIX of the two, and (B) the CURRENT LOCAL cost of each applicable connection for building a home (${new Date().getFullYear()}). Use an exact published amount when one exists; otherwise return a current source-backed budget range.
...
3) If PUBLIC WATER is NOT available, the current LOCAL low/high all-in cost to drill a private WELL (drilling + pump + connection) in this county.
4) If PUBLIC SEWER is NOT available, the current LOCAL low/high all-in cost of a conventional SEPTIC system (perc/soil test + install) in this county.
...
Return ONLY a JSON object in a \`\`\`json code block:
\`\`\`json
{ "publicWater": "available|not-available|unknown", "publicWaterSource": "https://...", "waterTap": 0, "waterTapLow": 0, "waterTapHigh": 0, "waterTapDetail": "", "waterTapSources": ["https://..."], "publicSewer": "available|not-available|unknown", "publicSewerSource": "https://...", "sewerTap": 0, "sewerTapLow": 0, "sewerTapHigh": 0, "sewerTapDetail": "", "sewerTapSources": ["https://..."], "well": 0, "wellLow": 0, "wellHigh": 0, "wellSources": ["https://..."], "septic": 0, "septicLow": 0, "septicHigh": 0, "septicSources": ["https://..."], "zoningPermitFee": 0, "zoningPermitLow": 0, "zoningPermitHigh": 0, "zoningPermitSources": ["https://..."], "drivewayPermitFee": 0, "drivewayPermitLow": 0, "drivewayPermitHigh": 0, "drivewayPermitSources": ["https://..."], "buildingPermitFee": 0, "buildingPermitLow": 0, "buildingPermitHigh": 0, "buildingPermitSources": ["https://..."], "permitNote": "", "provider": "", "sources": ["https://..."] }
\`\`\`
PRICING EVIDENCE RULES:
- For a fixed published fee, set the single-value field and repeat it as low/high.
- When no exact fee applies or the final amount depends on depth, soil, plans, meter size, or valuation, return a defensible low/high budget range synthesized from current local sources. Use nearby regional sources only when local pages do not publish numbers.
- Put every page supporting a line in its matching *Sources array. A number without a line-specific source URL is invalid and must be 0.
- Do not use national cost aggregators. Do not infer utility availability from city limits alone.
- A range is an estimate, not a quote. Keep it broad enough to contain the cited figures and never invent a midpoint.`;
  const utilitiesSystem = `You are a site-development utilities and permit-fee analyst for ${stateFull}. Use the supplied live research to find current address-level utility evidence, official fee schedules, and local or regional contractor pricing. Exact published fees and source-backed budget ranges are both useful, but must be distinguished in the JSON. Every number needs line-specific source URLs. Never use an uncited fallback or claim service availability from municipal limits alone.`;
...
Preserve supported values and fill the missing fields from this pass's new sources. Return one complete replacement JSON object.`;
  ...
  const lineSources = (...values: any[]): string[] => {
    ...
    for (const value of candidates) {
      ...
      try {
        ...
        if (NATIONAL_AGGREGATORS.some((host) => parsed.hostname.toLowerCase().includes(host))) continue;
        const grounded = evidencePool.length > 0
        ...
        if (grounded) supported.push(candidate);
      } catch { /* ignore malformed model URLs */ }
    }
    return [...new Set(supported)];
  };
  const lineSource = (...values: any[]): string | undefined => lineSources(...values)[0];
  ...
  type SourcedCost = Pick<UtilityLine, 'low' | 'high' | 'verified' | 'estimated' | 'sourceUrl' | 'sourceUrls'>;
  const sourcedCost = (input: {
    exactValue?: any;
    lowValue?: any;
    highValue?: any;
    sourceValues: any[];
    ...
    const sourceUrls = lineSources(...input.sourceValues);
    if (sourceUrls.length === 0) return { low: 0, high: 0, verified: false };
    const published = normalizeSourcedRange(input.exactValue, undefined, undefined);
    if (published && input.exactPublished) {
      return { low: published.low, high: published.high, verified: true, estimated: false, sourceUrl: sourceUrls[0], sourceUrls };
    }
    const range = normalizeSourcedRange(input.exactValue, input.lowValue, input.highValue);
    if (!range) return { low: 0, high: 0, verified: false };
    return { low: range.low, high: range.high, verified: false, estimated: true, sourceUrl: sourceUrls[0], sourceUrls };
  };

  const detailOf = (v: any) => { const s = String(v || '').trim(); return s ? s.slice(0, 120) : undefined; };
  const waterTapCost = sourcedCost({
    exactValue: o.waterTap,
    lowValue: o.waterTapLow,
    highValue: o.waterTapHigh,
    sourceValues: [o.waterTapSources, o.waterTapSource],
    exactPublished: true,
  });
  const sewerTapCost = sourcedCost({
    exactValue: o.sewerTap,
    lowValue: o.sewerTapLow,
    highValue: o.sewerTapHigh,
    sourceValues: [o.sewerTapSources, o.sewerTapSource],
    exactPublished: true,
  });
  const wellCost = sourcedCost({
    exactValue: o.well,
    lowValue: o.wellLow,
    highValue: o.wellHigh,
    sourceValues: [o.wellSources, o.wellSource],
  });
  const septicCost = sourcedCost({
    exactValue: o.septic,
    lowValue: o.septicLow,
    highValue: o.septicHigh,
    sourceValues: [o.septicSources, o.septicSource],
  });

  const lines: UtilityLine[] = [];
  if (publicWater === 'available') {
    lines.push({ name: 'Public water tap fee', kind: 'water', isPublic: true, status: 'available', ...waterTapCost, detail: detailOf(o.waterTapDetail), note: 'municipal water tap / connection / impact fee' });
  } else if (publicWater === 'not-available') {
    lines.push({ name: 'Private well', kind: 'water', isPublic: false, status: publicWater, ...wellCost, note: 'drill + pump + connection (no public water)' });
  } else {
    if (waterTapCost.low > 0) lines.push({ name: 'Public water tap scenario', kind: 'water', isPublic: true, status: 'unknown', scenario: true, ...waterTapCost, detail: detailOf(o.waterTapDetail), note: 'Budget only if the utility confirms a main and capacity at this address.' });
    if (wellCost.low > 0) lines.push({ name: 'Private well scenario', kind: 'water', isPublic: false, status: 'unknown', scenario: true, ...wellCost, note: 'Budget alternative if public water is not available.' });
    if (waterTapCost.low <= 0 && wellCost.low <= 0) {
      lines.push({ name: 'Water service', kind: 'water', isPublic: false, status: 'unknown', low: 0, high: 0, verified: false, sourceUrl: publicWaterSource, note: 'Address-level public water availability and a sourced cost range were not verified.' });
    }
  }

  if (publicSewer === 'available') {
    lines.push({ name: 'Public sewer tap fee', kind: 'sewer', isPublic: true, status: 'available', ...sewerTapCost, detail: detailOf(o.sewerTapDetail), note: 'municipal sewer tap / connection / impact fee' });
  } else if (publicSewer === 'not-available') {
    lines.push({ name: 'Septic system', kind: 'sewer', isPublic: false, status: publicSewer, ...septicCost, note: 'perc/soil test + conventional system install (no public sewer)' });
  } else {
    if (sewerTapCost.low > 0) lines.push({ name: 'Public sewer tap scenario', kind: 'sewer', isPublic: true, status: 'unknown', scenario: true, ...sewerTapCost, detail: detailOf(o.sewerTapDetail), note: 'Budget only if the utility confirms a main and capacity at this address.' });
    if (septicCost.low > 0) lines.push({ name: 'Septic system scenario', kind: 'sewer', isPublic: false, status: 'unknown', scenario: true, ...septicCost, note: 'Budget alternative if public sewer is not available; a soil evaluation is still required.' });
    if (sewerTapCost.low <= 0 && septicCost.low <= 0) {
      lines.push({ name: 'Sewer service', kind: 'sewer', isPublic: false, status: 'unknown', low: 0, high: 0, verified: false, sourceUrl: publicSewerSource, note: 'Address-level public sewer availability and a sourced cost range were not verified.' });
    }
  }

  const permits: PermitFeeLine[] = [];
  const zoningPermitCost = sourcedCost({
    exactValue: o.zoningPermitFee,
    lowValue: o.zoningPermitLow,
    highValue: o.zoningPermitHigh,
    sourceValues: [o.zoningPermitSources, o.zoningPermitSource],
    exactPublished: true,
  });
  permits.push({ name: 'Residential zoning permit', ...zoningPermitCost });
  const drivewayPermitCost = sourcedCost({
    exactValue: o.drivewayPermitFee,
    lowValue: o.drivewayPermitLow,
    highValue: o.drivewayPermitHigh,
    sourceValues: [o.drivewayPermitSources, o.drivewayPermitSource],
    exactPublished: true,
  });
  permits.push({ name: 'Driveway permit', ...drivewayPermitCost });
  const buildingPermitCost = sourcedCost({
    exactValue: o.buildingPermitFee,
    lowValue: o.buildingPermitLow,
    highValue: o.buildingPermitHigh,
    sourceValues: [o.buildingPermitSources, o.buildingPermitSource],
    exactPublished: true,
  });
  permits.push({
    name: 'Building + trade permits (new SFH, ~1,400-1,800 sqft)',
    ...buildingPermitCost,
    note: detailOf(o.permitNote) || (buildingPermitCost.low > 0
      ? 'Source-backed calculation; the final amount depends on plans and construction valuation.'
      : 'No sourced local or regional fee calculation was found.'),
  });
  ...
  if (!permits[2].verified && !permits[2].estimated) permits[2].note = 'No sourced local or regional fee calculation was found.';
  ...
  const real = lines.some((line) => line.verified || line.estimated) || permits.some((permit) => permit.verified || permit.estimated);
  // Unknown-service scenarios are mutually exclusive, so they are shown but not
  // added together into a misleading connection total.
  const totalLow = lines.filter((line) => !line.scenario).reduce((sum, line) => sum + line.low, 0);
  const totalHigh = lines.filter((line) => !line.scenario).reduce((sum, line) => sum + line.high, 0);
  ...
  const summary = availabilityUnknown
    ? 'Address-level utility availability is unverified; sourced public/private budget scenarios are shown where research found them.'
    : bothPublic
  ...
  const sources = [...new Set([
    ...localSources,
    ...lines.flatMap((line) => line.sourceUrls || []),
    ...lines.map((line) => line.sourceUrl).filter((url): url is string => !!url),
    ...permits.flatMap((permit) => permit.sourceUrls || []),
...
export async function chatWithGemini(
  ...
  const reportCompRadius = getCompPrefs().radiusMiles;
  const reportAllowedCompForms = reportData.compAllowedTypes?.length
    ? reportData.compAllowedTypes.map(residentialCompTypeLabel).join(', ')
    : 'the source-backed residential uses published for the parcel';
  const reportCompDateWindow = reportData.compDateWindow || getRollingCompDateWindow();
...
- Criteria already applied to that set: closed from ${reportCompDateWindow.soldSinceDate} through ${reportCompDateWindow.asOfDate}; new construction built ${reportCompDateWindow.minYearBuilt}-${reportCompDateWindow.maxYearBuilt}; exact property form matching the subject's zoning uses (${reportAllowedCompForms}); within ${reportCompRadius} driving miles; closest first.
- Keep single-family, mobile/manufactured, townhouse, condo, duplex, triplex, quadplex, 5+ unit multifamily, and multiple-residential-structure sales distinct. Never relabel a broad multifamily record as a duplex, triplex, or quadplex without published unit-count or subtype evidence.
...
- Adopted Standards Source: ${reportData.zoningStandardsSourceUrl || reportData.zoningSourceUrl || 'Use the official jurisdiction map linked in the zoning card.'}
- Net buildable envelope: ${reportData.gridics ? `${reportData.gridics.netBuildableAreaSqft.toLocaleString()} SF` : 'Calculate after official district confirmation.'}

### 4. SOLD New-Construction Comps (built ${reportCompDateWindow.minYearBuilt}-${reportCompDateWindow.maxYearBuilt}, zoning-use-matched, CLOSED sales from ${reportCompDateWindow.soldSinceDate} through ${reportCompDateWindow.asOfDate}, no sqft limits, within ${reportCompRadius} driving miles, RealtyAPI: Realtor + Redfin + Zillow)
- Zoning-permitted residential forms searched: ${reportAllowedCompForms}
${reportData.comps && reportData.comps.length > 0
  ? reportData.comps.map((comp, idx) => `- Comp ${idx + 1}: ${comp.address} | ${comp.propertyType || (comp.compType ? residentialCompTypeLabel(comp.compType) : 'Residential')} | ${comp.unitCount ? `${comp.unitCount} dwelling units | ` : ''}${comp.structureCount ? `${comp.structureCount} structures | ` : ''}Built ${comp.yearBuilt ?? 'N/A'} | ${comp.sqft ? `${comp.sqft.toLocaleString()} sqft | ` : ''}Sold ${comp.saleDate || 'N/A'} for $${comp.price.toLocaleString()}${comp.pricePerSqft ? ` ($${comp.pricePerSqft}/sqft)` : ''} | ${comp.distanceMiles.toFixed(2)} mi driving${comp.straightLineMiles != null ? ` (${comp.straightLineMiles.toFixed(2)} mi straight-line)` : ''}${comp.drivingFallback ? ' [straight-line fallback]' : ''} | Type evidence: ${comp.typeEvidence || 'RealtyAPI property-type field'} | Zoning match: ${comp.zoningMatchReason || 'source-backed permitted use'} | ${comp.verifiedNote || 'RealtyAPI sold record'}${comp.priceDiscrepancy ? ` | discrepancy: ${comp.priceDiscrepancy}` : ''}`).join('\n')
  : `NONE FOUND: no new-construction residential sale built ${reportCompDateWindow.minYearBuilt}-${reportCompDateWindow.maxYearBuilt}, matching the parcel's permitted forms (${reportAllowedCompForms}), closed from ${reportCompDateWindow.soldSinceDate} through ${reportCompDateWindow.asOfDate} inside the ${reportCompRadius}-mile driving radius (RealtyAPI: Realtor, Redfin, Zillow). Do NOT substitute older homes, vacant land, raw lots, unbuilt pads, active listings, or a different residential form. State plainly that no qualifying comps were available and note the county tax-assessor values as the only valuation reference.`}
